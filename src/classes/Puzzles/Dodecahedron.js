class Dodecahedron extends Puzzle {
    constructor(size = 1, width = 200) {
        ...
        const getCycleVector = attachedFace => faceConfig[attachedFace].points.reduce((v, p) => v.add(p), new Vector({x:0,y:0,z:0}));
        const cycleFamilyConfig = [];
        for (let c = 0; c < 12; c++) {
            let cycleConfig = { 'unitVector': getCycleVector(c), 'slices': { length: 5 } };
        ...
        cycleFamilyConfig.forEach((config, attachedFace) => {
            ...
            for (let i = 0; i < size; i++) {
                ...
                for (let j = 0; j < size; j++) {
                    ...
                    config.slices.forEach(slice => {
                        stickerCollection.push(stickerMap[`s-${slice.face}-${slice.startV}-${i}-${j}`]);
                        stickerCollection.push(stickerMap[`s-${slice.face}-${slice.endV}-${j}-${i}`]);
                    });
                    cycle.stickerCollections.push(stickerCollection);
                }
                let stickerCollection = [];
                config.slices.forEach(slice => {
                    stickerCollection.push(stickerMap[`s-${slice.face}-${slice.endV}-${i}-${size}`]);
                });
                cycle.stickerCollections.push(stickerCollection);
                cycle.stickerCollections.forEach(stickerCollection => stickerCollection.isPrimary = true);
                cycles.push(cycle);
            ...
            for (let i = 0; i < size; i++) {
                for (let j = 0; j <= size; j++) {
                    let stickerCollection = [];
                    for (let p = 0; p < 5; p++) {
                        stickerCollection.push(stickerMap[`s-${attachedFace}-${p}-${i}-${j}`]);
                    };
