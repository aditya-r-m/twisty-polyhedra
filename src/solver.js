(() => {
    ...
    window.updateSolutionPanel = () => {
        let sequence = solutionStack.movesToMake.length
            ? solutionStack.movesToMake[solutionStack.movesToMake.length - 1].sequence : 'Solved!';
        let subSequence = solutionStack.movesToMake.length
            ? solutionStack.movesToMake[solutionStack.movesToMake.length - 1].subSequence : '';
        window.solutioninfo.innerHTML = `
            ${sequence}
            <br/>
            ${subSequence ? `${subSequence}<br/>` : ''}
            ${solutionStack.movesMade.length}/${solutionStack.movesMade.length + solutionStack.movesToMake.length}
        `;
    }
    ...
    window.generateSolution = () => {
        ...
        window.solverstatustext.innerHTML = 
    ...
    window.makeSolverMove = () => {
        ...
        window.selectedPuzzle.cycles[moveToMake.cycleIndex].twist(moveToMake.direction);
    ...
    window.revertSolverMove = () => {
        ...
        window.selectedPuzzle.cycles[moveToRevert.cycleIndex].twist(-moveToRevert.direction);
        solutionStack.movesToMake.push(moveToRevert);
        updateSolutionPanel();
    }
    window.makeSolverMoveSequence = () => {
        if (!solutionStack.movesToMake.length) return;
        let sequenceToCover = solutionStack.movesToMake[solutionStack.movesToMake.length - 1].sequence;
        while (solutionStack.movesToMake.length
            && solutionStack.movesToMake[solutionStack.movesToMake.length - 1].sequence === sequenceToCover) {
            let moveToMake = solutionStack.movesToMake.pop();
            window.selectedPuzzle.cycles[moveToMake.cycleIndex].twist(moveToMake.direction);
            solutionStack.movesMade.push(moveToMake);
        }
    ...
    window.revertSolverMoveSequence = () => {
        if (!solutionStack.movesMade.length) return;
        let sequenceToCover = solutionStack.movesMade[solutionStack.movesMade.length - 1].sequence;
        while (solutionStack.movesMade.length
            && solutionStack.movesMade[solutionStack.movesMade.length - 1].sequence === sequenceToCover) {
            let moveToRevert = solutionStack.movesMade.pop();
            window.selectedPuzzle.cycles[moveToRevert.cycleIndex].twist(-moveToRevert.direction);
            solutionStack.movesToMake.push(moveToRevert);
        }
